import { readJsonSync } from "https://deno.land/std@0.65/fs/read_json.ts";
...
function extractDependencyAndVersion(imports: object): dependencyType[] {
  ...
  for (const [key, value] of Object.entries(imports)) {
    ...
    const exactVersion = (substr.indexOf("/") > -1) ? substr.substring(0, substr.indexOf("/")) : substr;
    // In case if there is an extra backslash at the end of the dependency name
    const exactName = (key.indexOf('/') > -1) ? key.substring(0, key.lastIndexOf('/')) : key;
    ...
    if (exactName && moduleName && value && exactVersion) {
      ...
      console.warn('Could not find all information about module ', value, ' Skipping module.');
    }
  }
  return result;
}
